...
var scene, renderer, camera, model, mixer, idle, newAnim, idle_anim, fileAnimations;
let idx = 0;
...
let MODEL_PATH = "static/decimated-vantom-v1.glb";
MODEL_PATH = "static/Stork.glb";
MODEL_PATH = "static/vantom-v1.glb";
...
camera.lookAt(model.position);
    ...
    // let idle_action = mixer.clipAction(action_clip);
    // idle_action.play();
    scene.add(model);
  },
  undefined,
...
sock.on('user', function(event){
    ...
    ak = mixer.clipAction(A_key);
    ok = mixer.clipAction(O_key);
    ek = mixer.clipAction(E_key);
    bpmk = mixer.clipAction(BPM_key);
    fvk = mixer.clipAction(FV_key);
    wqk = mixer.clipAction(WQ_key);
...
//    }); // END API
    ...
    mixer.addEventListener("finished", function(e){
        if(wi >= phoneme.length){
                ...
                ra = mixer.clipAction(reset_action);
                ra.fadeIn(1);
                ra.play();
